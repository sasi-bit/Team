...
from .models import User, Bid, Item, ItemComment, Watchlist, ItemCategory, AuctionHistory,balance
...
def profile(request):
    ...
    if request.method == "POST":
        try:
            ...
            check=User.objects.filter(username=request.user.username)
    ...
    user = balance.objects.filter(user=request.user).first()

    return render(request,"auctions/profile.html",{"balance":user.balance,"msg":msg})
